...
export default function Sidebar({ isConnected, open = false, onClose = () => {} }) {
  return (
    <>
      {/* Mobile backdrop */}
      {open && (
      ...
      <aside
        className={`fixed left-0 top-0 bottom-0 w-64 glass-card rounded-none border-r border-t-0 border-b-0 border-l-0 border-r-white/[0.06] flex flex-col z-50 transition-transform duration-300 ease-fluid
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 group relative ${
                  isActive
                    ? 'text-neon-cyan bg-neon-cyan/10 border-l-2 border-neon-cyan shadow-[inset_0_0_20px_rgba(34,211,238,0.06)]'
                ...
                <>
                  <Icon
                    className={`w-[18px] h-[18px] transition-all duration-200 ${
                      isActive ? 'text-neon-cyan drop-shadow-[0_0_6px_rgba(34,211,238,0.5)]' : 'text-gray-500 group-hover:text-gray-300'
                    }`}
                  />
                  <span className="font-heading tracking-wide">{label}</span>
                  {isActive && (
                    <div className="absolute right-2 w-1.5 h-1.5 rounded-full bg-neon-cyan shadow-[0_0_8px_rgba(34,211,238,0.8)]" />
                  )}
                </>
              )}
            </NavLink>
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${
                  isConnected
                    ? 'bg-neon-green shadow-[0_0_8px_rgba(52,211,153,0.6)]'
                    : 'bg-neon-red shadow-[0_0_8px_rgba(251,113,133,0.6)]'
                }`}
              />
